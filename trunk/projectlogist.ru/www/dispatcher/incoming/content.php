		<script type="text/javascript">
			...
			var Field = function(information) {
				...
				switch (information.type) {
					case 'ERROR': {
						document.getElementById('status-error').appendChild(this.DOMObj);
						if (information.hasPreviousRoutePoint) {
							this.buttons = [new ButtonObject(new Type('COME_IN'),this),new ButtonObject(new Type('ERROR'),this), new ButtonInfo(this)];
						} else {
					...
					case 'LOADING': {
						...
						this.buttons = [new ButtonObject(new Type('LOADING'),this),new ButtonObject(new Type('ERROR'),this), new ButtonInfo(this)];
						break;
					}
				}
				//functions
			...
			Type = function(name) {
				var typeArray = {ERROR:'ERROR',COME_IN:'COME_IN',LOADING:'LOADING'};
				var typeAJAXString = {ERROR:'setMistakeStatusForInvoice',COME_IN:'setLoadingStatusForInvoice',LOADING:'setLeavingStatusForInvoice'};
				var typeButtonString = {ERROR:'ошибка',COME_IN:'прибытие',LOADING:'отправка'};
				if (!typeArray[name]) {
					throw new Error('wrong value for enumerate. ERROR, COME_IN or LOADING expected');
				}
