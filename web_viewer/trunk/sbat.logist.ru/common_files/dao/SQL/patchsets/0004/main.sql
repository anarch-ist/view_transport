...
CREATE PROCEDURE transmaster_transport_db.selectRoutes(IN `_startEntry` INT, IN `_length` INT,
  ...
  BEGIN
    ...
    WHERE (
      ...
      t.cost_per_hour LIKE @searchString collate utf8_general_ci OR
      t.cost_per_box LIKE @searchString collate utf8_general_ci
    )
    ORDER BY NULL,
      CASE WHEN _orderby = ''
        THEN NULL END,
      ...
      CASE WHEN `_isDesc` AND `_orderby` = 'cost_per_box'
        THEN cost_per_box END ASC,
      ...
      CASE WHEN NOT (_isDesc) AND _orderby = 'cost_per_box'
        THEN cost_per_box END DESC
