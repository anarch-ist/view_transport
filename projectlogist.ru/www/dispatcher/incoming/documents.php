<?php
	...
	function main() {
		...
		else if ($_POST['status']==='setLeavingStatusForInvoice') {
			$StatusID = setLeavingStatusForInvoice($_POST['InvoiceID']);
			if (isset($StatusID)) {getNewStatus($StatusID);}
			return;
		}
	...
	function setLoadingStatusForInvoice($currID) {		// set loading status for $currID object
		...
		$result = changeStatus('0303',$currID,$datetime->format('Y-m-d H:i:s'),$_POST['expectedEndStatusTime'],$date->modify('+'.$timeForAction['tLoading'].' minutes')->format('Y-m-d H:i:s'),$_POST['routePointID'],$_POST['routePointID']);
		if ($result) {
			disablePrevStatus();
			return getIndex();
		}
	...
	function setLeavingStatusForInvoice($currID) {		// set 'come in next point' status for $currID object. 
		...
		$result = changeStatus('0302',$currID,$datetime->format('Y-m-d H:i:s'),$_POST['expectedEndStatusTime'],$date->modify('+'.$timeForAction['tToNextPoint'].' minutes')->format('Y-m-d H:i:s'),$_POST['routePointID'],getNextPoint($_POST['routePointID']));
		if ($result) {
			disablePrevStatus();
			return getIndex();
		}
	...
	function setMistakeStatusForInvoice($currID) {		// set 'error' status for $currID object. 
		...
		if ($result===false) {
			...
			$result = changeStatus('0304',$currID,$datetime->format('Y-m-d H:i:s'),$_POST['expectedEndStatusTime'],$date->modify('+'.$timeForAction['tToNextPoint'].' minutes')->format('Y-m-d H:i:s'),$_POST['routePointID'],getPrevPoint($_POST['routePointID']));
		}
		if ($result) {
			disablePrevStatus();
			return getIndex();
	...
	function changeStatus($statusType, $invoiceID, $StatusStartDate, $ExpectedStatusStartDate,$ExpectedStatusEndDate, $RoutePointID, $NextRoutePointID) {
